...
class GPURolloutBuffer(BaseBuffer):
    """
    Rollout buffer used in on-policy algorithms like A2C/PPO compatible with the GPUVecEnv.
    It corresponds to ``buffer_size`` transitions collected
    using the current policy.
    This experience will be discarded after the policy update.
    In order to use PPO objective, we also store the current value of each state
    ...
    episode_starts: th.FloatTensor
    log_probs: th.FloatTensor
    ...
    def add(self,
        ...
        assert action.shape[0]==self.n_envs and action.shape[1]==self.action_dim, "Check action dim"

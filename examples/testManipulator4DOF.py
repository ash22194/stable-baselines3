...
sys = {'m': m, 'l': l, 'g': g,\
	   ...
	   'T': 12, 'dt': 1e-3, 'lambda_': 3, 'X_DIMS': 8, 'U_DIMS': 4,\
...
if ((save_timestep <= total_timesteps) and (save_timestep > 0)):
	...
	if (algorithm == 'A2C'):
		policy_kwargs = dict(activation_fn=nn.ReLU, net_arch=[dict(pi=[512, 512], vf=[512, 512])], log_std_init=policy_std, optimizer_class=RMSpropTFLike, optimizer_kwargs=dict(eps=1e-5))
